...
class admin_plugin_doctree2filelist extends DokuWiki_Admin_Plugin
{
    ...
    function html()
    {
        ...
        print  $this->getLang('importdir') . ": <code>" . $file_upload . "</code><br />\n";
        print  $this->getLang('targetns') . ": <code>" . $this->getConf('destination_namespace') . "</code>\n";
        print "</div>\n";
        ...
        if (is_dir($file_upload) && ($status != "IMPORTED") && ($status != "DOCSUPLOADED")) {
            print "<span class=\"ospok\">OK </span>" . $this->getLang('sourcedir_exists') . " <code> " . $file_upload . " </code></span>\n";
